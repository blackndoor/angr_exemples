...
# load the file
project = angr.Project('crackme64')
...
state = project.factory.path(args=["./crackme64",argv1])

# start the symbolic execution, create a PathGroup
group = project.factory.path_group(state)

# explore to the "well done" state (0x4005dc found with IDA)
# avoiding the "get out" state (0x4005e8 found with IDA)
group.explore(find=0x4005dc, avoid=0x4005e8)
